...
void PartFetcherActionServer::executeCB(const actionlib::SimpleActionServer<part_fetcher::PartFetcherAction>::GoalConstPtr& goal) {
     ...
     //send fetch and dropoff requests to object_grabber_action_server
...
int main(int argc, char** argv) {
	...
	if (g_object_ID = ObjectIdCodes::TOY_BLOCK_ID){
	    system("./request_block.sh");
	}else{
	    system("./request_pulley.sh");
	}
